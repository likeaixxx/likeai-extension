...
import { getPreferenceValues, showHUD, closeMainWindow, LaunchProps, Detail, ActionPanel, Action, Icon, List, open } from "@raycast/api";
...
async function getSongInfo() {
 try {
  const script = `
    ...
    `;
  const result = JSON.parse(execSync(`osascript -e '${script.replace(/'/g, "'\\''")}'`).toString().replaceAll("'", '"').trim());
...
export default function (props: LaunchProps<{ arguments: Arguments.Refresh }>) {
  ...
  const fetchCurrentSongLyrics = async () => {
   try {
    ...
    try {
     ...
     const errorMessage = geniusError instanceof Error ? geniusError.message : "Failed to fetch lyrics. Please try again.";
 ...
 if (isLoading || lyrics.length === 0) {
  return <Detail markdown={`# Loading lyrics for "${(songInfo?.title + "" + songInfo?.artist) || "current song"}"...`} isLoading />;
 }
    ...
    actions={
     <ActionPanel>
      ...
      <Action.OpenInBrowser
       title="Search Lyrics with Browser" url={`https://www.google.com/search?q=${songInfo?.title + " · " + songInfo?.artist}`} />
      <Action
       icon={Icon.ArrowClockwise}
       title="Refresh"
       onAction={() => open(`raycast://extensions/like-ai/likeai-extension/findLyrics?arguments=%7B%22refresh%22%3A%22true%22%7D`)}
      />
     </ActionPanel>
    }
   />
     ...
     <List.Item
      ...
      actions={
       <ActionPanel>
        {
         refresh &&
         <Action icon={Icon.Checkmark} title="Confirm"
          onAction={async () => {
           await conformApi(lyrics_api, { item: lyric })
           await showHUD("Lyrics confirmed")
           await open(`raycast://extensions/like-ai/likeai-extension/findLyrics?arguments=%7B%22refresh%22%3A%22%22%7D`)
          }} />
        }
        <Action icon={Icon.ArrowClockwise} title="Refresh"
         onAction={() => open(`raycast://extensions/like-ai/likeai-extension/findLyrics?arguments=%7B%22refresh%22%3A%22true%22%7D`)} />
        <Action.CopyToClipboard
         title="Copy Lyrics"
         content={markdown(lyric.lyrics)}
